...
router.post('/', upload.single('image'), async (req, res) => {
  try {
    const { facultyId, departmentId, lastname, firstname, jambRegNumber, matNumber, phoneNumber, yearOfAdmission } = req.body;

    // Check if all required fields are provided
    if (!facultyId || !departmentId || !lastname || !firstname || !jambRegNumber || !matNumber || !phoneNumber || !yearOfAdmission) {
    ...
    doc.fontSize(16).text(`Submission for ${firstname} ${lastname} ${lastname}`, { underline: true });
    ...
    doc.text(`Mat Number: ${matNumber}`);
    doc.text(`Jamb Reg Number: ${jambRegNumber}`);
